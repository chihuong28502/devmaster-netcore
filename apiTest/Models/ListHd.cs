using System;
using System.Collections.Generic;

namespace apiTest.Models;

public partial class ListHd
{
    public string? HosoTdId { get; set; }

    public string? SoHopDong { get; set; }

    public string? TkTinDung { get; set; }

    public string? TenNguoiVay { get; set; }

    public string? NgayVay { get; set; }

    public string? NgayDaoHan { get; set; }

    public double? LaiSuat { get; set; }

    public double? SoDu { get; set; }

    public string? KhuvucMa { get; set; }

    public string? KhuvucTen { get; set; }

    public string? TenTaiKhoan { get; set; }

    public int? NhomNo { get; set; }

    public string? MucDichVayId { get; set; }

    public string? MucDichVay { get; set; }

    public string? NganhNgheKdMa { get; set; }

    public string? NganhNgheKdTen { get; set; }

    public string? SanPhamTdId { get; set; }

    public string? HinhThucTstcId { get; set; }

    public string? HinhThucTstcTen { get; set; }

    public double? TstcTriGia { get; set; }

    public int? LoaiVayId { get; set; }

    public string? MaThongKe0 { get; set; }

    public string? MaThongKe1 { get; set; }

    public string? TenNhomTk1 { get; set; }

    public string? TenNhomTk0 { get; set; }

    public string? NgayGiaHan { get; set; }

    public string? LoaiHinhKinhTeId { get; set; }

    public double LaiDaThu { get; set; }

    public double SoDuThu { get; set; }

    public string? KhachHangId { get; set; }

    public double? NganHan { get; set; }

    public double? TrungHan { get; set; }
}
